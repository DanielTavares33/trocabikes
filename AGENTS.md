...
- E2E in Docker: run `make e2e-setup` once (installs Playwright Chromium + builds assets), then `make e2e`. Headed/UI/debug runs must be on the **host** (`bun run test:e2e:headed`); Docker has no display. Slow-mo is set in `playwright.config.ts` for headed runs. On the host: `make e2e-fix-perms`, `bunx playwright install chromium`, then `bun run test:e2e:headed -- --grep "Buyer signs in"`. Skip `bun run build` if Docker already built `public/build`. Use `bun run test:e2e:debug` to pause the Inspector.
