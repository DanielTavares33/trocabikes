...
test('login rate limit resets after decay period', function () {
    User::factory()->create([
        'email' => TEST_EMAIL,
        'password' => Hash::make(TEST_PASSWORD),
    ]);
    ...
    // Clear the rate limiter (simulate time passing)
    RateLimiter::clear('login:'.request()->ip());

    // Should be able to login successfully now
    $response = post(route('login'), [
        'email' => TEST_EMAIL,
        'password' => TEST_PASSWORD,
    ]);
...
test('registration is rate limited after 5 failed attempts', function () {
    // Make 5 failed registration attempts (invalid data)
    for ($i = 0; $i < 5; $i++) {
        post(route('register'), [
            'name' => '',
...
test('registration rate limit returns retry-after header', function () {
    // Make 5 failed registration attempts
    for ($i = 0; $i < 5; $i++) {
        post(route('register'), [
            'name' => '',
            'email' => "user{$i}@example.com",
...
test('successful registration does not count towards rate limit', function () {
    ...
    // 6th registration should still work (successful ones don't count)
    $response = post(route('register'), [
        'name' => USER_NAME,
        'email' => USER_EMAIL,
        'password' => 'password123',
