    ...
    <div className="flex flex-1 flex-col">
      <div className="mb-4 flex items-center justify-between gap-4">
        <p className="text-sm text-text-muted">
          <span className="font-medium text-text">{total}</span> bikes found
        </p>

        <select
          aria-label="Sort bikes"
          data-testid="bike-sort"
          value={sortValue}
      ...
      {bikes.length === 0 ? (
        ...
        <div
          ...
          className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-[repeat(auto-fill,minmax(300px,1fr))]"
        >
          {bikes.map((bike) => (
